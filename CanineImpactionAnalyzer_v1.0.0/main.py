...
LANDMARK_SEQUENCE = [
    ...
    ("mid1", "MIDLINE: click first point"),
    ("mid2", "MIDLINE: click second point"),
    ...
    ("occ1", "OCCLUSAL PLANE: click first point"),
    ("occ2", "OCCLUSAL PLANE: click second point"),

    ("cal1", "CALIBRATION: click first point"),
    ("cal2", "CALIBRATION: click second point"),
]
...
class ImageView(QGraphicsView):
    ...
    def mousePressEvent(self, event):
        # Left click records a landmark. Right/middle still allow navigation.
        if (
...
PURPOSE
This application is a research tool for standardized measurement of
maxillary canine position on panoramic radiographs.

GETTING STARTED
1. Click "Open Panoramic Image" and select the radiograph.
2. Enter the Patient ID and age.
3. Select the root-development stage for the right and left canines.
4. Enter the known calibration distance in millimeters.
5. Click "Start Landmark Measurement".
6. Follow the instructions shown on the left side of the screen.

LANDMARK SEQUENCE
1. Right canine cusp tip
2. Right canine apex
3. Left canine cusp tip
4. Left canine apex
5. First maxillary midline point
6. Second maxillary midline point

RIGHT SIDE
7. Distal border of lateral incisor
8. Crown midpoint of lateral incisor
9. Apex of lateral incisor
10. Distal border of central incisor
11. Crown midpoint of central incisor
12. Apex of central incisor

LEFT SIDE
13. Distal border of lateral incisor
14. Crown midpoint of lateral incisor
15. Apex of lateral incisor
16. Distal border of central incisor
17. Crown midpoint of central incisor
18. Apex of central incisor

REFERENCE LINES
19. First occlusal-plane point
20. Second occlusal-plane point
21. First calibration point
22. Second calibration point
...
• Alpha angle (α): canine long axis relative to the maxillary midline
• Canine-occlusal angle
• Distance D in pixels and calibrated millimeters
• Five-sector canine position
• Root-development stage

CONTROLS
• Mouse wheel: zoom in/out
• Drag: pan across the image
• Undo Last Landmark: remove the most recent point
• Reset Landmarks: clear the current measurements
• Save Result to CSV: export measurements and landmark coordinates

IMPORTANT
Place each landmark as precisely as possible. If the application warns
that sector landmarks are not in the expected anatomical order, review
and repeat those landmarks.

The eruption assessment and recommendation currently shown by the
application are provisional research logic and are not yet a validated
clinical decision rule. The software should not be used as a substitute
for professional clinical diagnosis or treatment planning.
""".strip()
    ...
    def undo_last(self):
        ...
        if self.image_path:
            ...
            for i in range(self.click_index):
                ...
                if k in old_points:
                    self.draw_marker(old_points[k], i + 1)
    ...
    def draw_marker(self, point, number):
        """
        Draw a small landmark marker, similar in size to the original
        matplotlib/Python prototype.
        """
        x, y = point

        # Small image-space marker.
        radius = 3.25
        ...
        # Keep the landmark number very small so it does not cover anatomy.
        text = self.view.scene().addText(str(number))
