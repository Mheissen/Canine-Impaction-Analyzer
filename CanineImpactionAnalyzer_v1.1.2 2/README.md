# Canine Impaction Analyzer v1.1
...
- Cross-platform display consistency: measurement lines use cosmetic pens so their visible thickness is the same on Windows and macOS and remains stable while zooming.
- Landmark points have a constant screen size on Windows and macOS.
- Every landmark can be dragged after placement; connected lines and measurements update automatically.
- Changing age, canine root-development stage, or calibration distance automatically recalculates the results.
- Midline guidance: first point between the maxillary central-incisor incisal edges; second point at approximately the CEJ level of the maxillary central-incisor crowns.
- Occlusal-plane guidance: mesial cusp of one upper first molar to the corresponding mesial cusp of the opposite upper first molar.
- Calibration guidance: prefer a radiographic ruler/marker or validated metadata; otherwise use a reference dimension independently known for that image/patient, such as a measured central-incisor crown dimension or molar crown width.
- Image import filters now include JPEG/JPG, PNG, TIFF, BMP, and WebP.
